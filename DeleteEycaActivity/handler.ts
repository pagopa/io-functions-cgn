...
import { RetrievedUserEycaCard, UserEycaCardModel } from "../models/user_eyca_card";
...
export const DeleteEycaActivityResultSuccess = t.intersection([CommonActivityResultSuccess,t.interface({cards: t.readonlyArray(RetrievedUserEycaCard)})]);

export type DeleteEycaActivityResultSuccess = t.TypeOf<typeof DeleteEycaActivityResultSuccess>;
...
export type DeleteEycaActivityResult = t.TypeOf<typeof DeleteEycaActivityResult>;
...
export const getDeleteEycaActivityHandler = (
  ...
  logPrefix: string = "DeleteEycaActivity"
    ...
    return fromEither(ActivityInput.decode(input))
      ...
      .chain(
        activityInput => userEycaModel.findAll(activityInput.fiscalCode)
        .mapLeft( _ => fail(_, "Cannot retriew all eyca card") )
      )
      .chain( 
        cards => 
          array.sequence(taskEither)(cards.map( element => userEycaModel.deleteVersion(element.fiscalCode, element.id) ))
          .mapLeft( _ => fail(_, "Cannot delete eyca version")  )
          .map( () => cards )
      )
      .fold<DeleteEycaActivityResult>(identity, cards => ({ kind: "SUCCESS", cards }))
      .run();
